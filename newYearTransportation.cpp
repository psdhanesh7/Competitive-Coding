...
void dfs(vector<int> *edges, int currVertex, bool *visited) {
	...
	for(int i = 0; i < edges[currVertex].size(); i++) {
		...
		if(!visited[adjacent]) {
			dfs(edges, adjacent, visited);
		}
	}
}

bool pathAvailable(vector<int> *edges, int n, int t) {
